...
import com.xingyun.equipment.crane.modular.device.dao.ProjectLiftVideoMapper;
import com.xingyun.equipment.crane.modular.device.dto.ProjectLiftDTO;
...
import com.xingyun.equipment.crane.modular.infromation.service.impl.ProjectTargetSetLiftServiceImpl;
...
public class ProjectLiftServiceImpl extends ServiceImpl<ProjectLiftMapper, ProjectLift> implements IProjectLiftService {
    ...
    public boolean updateProjectLift(RequestDTO<ProjectLiftDTO> res) throws Exception{
        boolean isSuccess = false;
        String key = Const.DEVICE_INFO_PREFIX+"lift:" + res.getBody().getDeviceNo();
    ...
    public ResultDTO<Object> updateStatus(RequestDTO<ProjectLiftDTO> res) {
        ...
        for(int i=0;i<res.getIds().size();i++) {
            ...
            String key = Const.DEVICE_INFO_PREFIX+"lift:" + deviceNum;
            if(redisUtil.exists(key)) {
                redisUtil.remove(key);
            }
            ProjectLift projectLift = new ProjectLift();
            if("stop".equals(res.getKey())){//是停用
                String redisKey = Const.DEVICE_PLATFORM+"lift:" + deviceNum;
    ...
    public boolean deleteBatchIds(List<? extends Serializable> idList) {
        for(int i=0;i<idList.size();i++){
            ...
            String key = Const.DEVICE_INFO_PREFIX+"lift:" + baseMapper.selectById(idList.get(i)).getDeviceNo();
            if(redisUtil.exists(key)) {
                redisUtil.remove(key);
            }
