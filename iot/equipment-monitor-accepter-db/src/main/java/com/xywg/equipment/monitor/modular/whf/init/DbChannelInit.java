 ...
 */
@Component
public class DbChannelInit extends ChannelInitializer<SocketChannel> {

    private static final Logger LOGGER = LoggerFactory.getLogger(DbChannelInit.class);
    /**存储所有通道  ， 用于向设备回消息
    ...
    @Autowired
    private DbHandler dbDataHandler;
    @Autowired
    ...
    protected void initChannel(SocketChannel ch) throws Exception {
        ...
        ch.pipeline()
                .addLast(new DbDecoder())
                //心跳检测  60秒未收到数据，服务器主动断开连接
                .addLast(new IdleStateHandler(60, 0, 0, TimeUnit.SECONDS))
                .addLast(dbDataHandler,linkSNHandler);
