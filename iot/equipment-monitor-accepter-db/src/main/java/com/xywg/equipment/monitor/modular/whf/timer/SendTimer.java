...
import com.xywg.equipment.monitor.modular.whf.handler.DbHandler;
...
public class SendTimer {
    ...
    public void send(){
        ...
        for(String sn:channels.keySet()){
           ...
           if(!channel.isActive()){
               ...
               DbChannelInit.sendMessage.put(sn,sendStr);
    ...
    private String getRealAddr(String addr){
        ...
        //高位补0
        if(addr.length()< DbDecoder.LINK_SN_LENGTH){
            for (int i = 0; i < DbDecoder.LINK_SN_LENGTH-addr.length(); i++) {
                addr = addr+"0";
            }
        }
        return addr;
    ...
    private String addChangeNum(String msg){
        msg = Integer.toHexString(Integer.parseInt(msg,16)+ DbHandler.CHANGE_NUM);
        return msg;
    }
    /**
     * 地址顺序去反
