...
import com.xywg.equipment.monitor.modular.whf.handler.NettyHandler;
...
public class ZbusProducerHolder {
    ...
    private Producer xldataProducer;
    private Producer xlhtProducer;
    private Producer xllxProducer;
    ...
    public void init(String host) {
        try {
            ...
            // 卸料心跳生产者
            xlhtProducer=new Producer(broker);
            xlhtProducer.declareTopic(zbusConfig.getXlHtTopic());

            // 卸料数据生产者
            xldataProducer=new Producer(broker);
            xldataProducer.declareTopic(zbusConfig.getXlDataTopic());

            //卸料离线生产者
            xllxProducer = new Producer(broker);
            xllxProducer.declareTopic(zbusConfig.getXlLxTopic());
            ...
            consumer.start(new MessageHandler() {
                ...
                public void handle(Message message, MqClient mqClient) throws IOException {
                    ...
                    if("01".equals(controllerDTO.getCmd())){
                        ...
                        Channel channel =  NettyHandler.nettyChannels.get(localKey);
                        if(channel!=null){
                            channel.close();
                        }
    ...
    public void sendXlHtMessage(String data) throws Exception{
        Message msg=new Message();
        msg.setHeader("server",serverProperties.getName());
        msg.setTopic(zbusConfig.getXlHtTopic());
        msg.setBody(data);
        xlhtProducer.publish(msg);
    }
    public void sendXlDataMessage(String data) throws Exception{
        Message msg=new Message();
        msg.setHeader("server",serverProperties.getName());
        msg.setTopic(zbusConfig.getXlDataTopic());
        msg.setBody(data);
        tjdataProducer.publish(msg);
    }
    public void sendXlLxMessage(String data) throws Exception{
        Message msg=new Message();
        msg.setHeader("server",serverProperties.getName());
        msg.setTopic(zbusConfig.getXlLxTopic());
        msg.setBody(data);
    ...
    public void sendTestMessage(String sn ,String data) throws Exception{
        Message msg=new Message();
        msg.setHeader("Device-Type","crane");
        msg.setHeader("Vender","anhuijuzheng");
        msg.setHeader("SN",sn);
