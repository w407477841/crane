...
import com.xywg.equipment.monitor.modular.sb.dto.SbDataDTO;
...
public class SbHandler extends SimpleChannelInboundHandler<SbDataDTO> {

    private static final Logger LOGGER = LoggerFactory.getLogger(SbHandler.class);
     ...
     * 发送需要 加上
     * */
    public static final int CHANGE_NUM = 0;

    @Autowired
    ZbusProducerHolder  zbusProducerHolder;
    @Override
    protected void channelRead0(ChannelHandlerContext ctx, SbDataDTO msg) throws Exception {
        LOGGER.info("##############数据处理器数据###################");
        ...
        String send = SbChannelInit.sendMessage.get(addr);
    ...
    public void channelInactive(ChannelHandlerContext ctx) throws Exception {
        ...
        if(session!=null) {
            ...
            for(int i = 0;i < list.size();i++) {
                ...
                SbChannelInit.channels.remove(list.get(i));
                if (localKey.startsWith(type_water)) {
                    zbusProducerHolder.sendDbLxMessage(localKey);
                }
            }
        }
        LOGGER.info("###########有设备断开水表数据处理器############");
        /*//删除本地缓存
        ...
        for(String sn:channels.keySet()){
            if(channels.get(sn).remoteAddress().toString().equals(ctx.channel().remoteAddress().toString())){
                ...
                SbChannelInit.channels.remove(sn);
            }
        }*/
