...
import com.xingyun.equipment.admin.modular.device.dao.ProjectWaterMeterMapper;
...
import com.xingyun.equipment.admin.modular.device.service.IProjectWaterMeterService;
import com.xingyun.equipment.admin.modular.device.vo.DataVO;
import com.xingyun.equipment.admin.modular.device.vo.MonitorAlarmVO;
import com.xingyun.equipment.admin.modular.device.vo.ProjectWaterMeterAlarmVo;
import com.xingyun.equipment.admin.modular.device.vo.ProjectWaterMeterDetailVo;
import com.xingyun.equipment.admin.modular.device.vo.ProjectWaterMeterHeartbeatVo;
import com.xingyun.equipment.admin.modular.device.vo.ProjectWaterMeterVo;
import com.xingyun.equipment.admin.modular.device.vo.WaterAlarmVO;
import com.xingyun.equipment.admin.modular.infromation.dao.ProjectTargetSetWaterMapper;
...
//            		amount = amount.add(amount1);
    ...
    public boolean updateById(ProjectWaterMeter entity) {
        ...
        String key = RedisConfig.DEVICE_INFO_PREFIX + "water:" + entity.getDeviceNo();
    ...
    public boolean deleteBatchIds(List<? extends Serializable> idList) {
        ...
        for (int i = 0; i < list.size(); i++) {
            String key = RedisConfig.DEVICE_INFO_PREFIX + "water:" + list.get(i).getDeviceNo();
            redisUtil.remove(key);
        }
        return isSuccess;
    }
    ...
    public boolean setUse(RequestDTO<ProjectWaterMeter> requestDTO) throws Exception {
        ...
        if (this.updateBatchById(list)) {
            ...
            for (int i = 0; i < projectWaterMeters.size(); i++) {
                String key = RedisConfig.DEVICE_INFO_PREFIX + "water:" + projectWaterMeters.get(i).getDeviceNo();
                redisUtil.remove(key);
            }
        }
        return isSuccess;
