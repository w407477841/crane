...
public class CommonMethod {
    ...
    public void resMessageJoint(Channel channel, String sn16, LinkedHashMap<String, String> map, String command, String protocolVersion, String aQ) {
        ...
        //拼接最终16进制字符串
        String message = messageJoint(sn16,  map, command, protocolVersion, aQ);
        channel.writeAndFlush(Unpooled.copiedBuffer(toBytes(message)));

        logger.info("System Reply:<<Device:" + decode(sn16) + " Message:" + message + ">>");
    }

    public String messageJoint( String sn16, LinkedHashMap<String, String> map, String command, String protocolVersion, String aQ) {
    ...
    public void send2Mq(String data){
        rabbitTemplate.convertAndSend(RabbitConfig.CRANE_EXCHANGE,RabbitConfig.CRANE_BINDINGDS[1][1],data);
    }
}
