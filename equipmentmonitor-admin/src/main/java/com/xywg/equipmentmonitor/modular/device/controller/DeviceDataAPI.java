...
import com.xywg.equipmentmonitor.modular.device.dto.CraneDetailDTO;
import com.xywg.equipmentmonitor.modular.device.dto.LiftDetailDTO;
import com.xywg.equipmentmonitor.modular.device.dto.MonitorDetailDTO;
...
public class DeviceDataAPI {
    ...
    @PostMapping("monitor/insertData")
        ...
        try {
            ...
            if(monitor.getNoise()!=null){
              BigDecimal newNoise =  monitor.getNoise().multiply(new BigDecimal("10"));
                monitor.setNoise(newNoise);
            }
    ...
    public ResultDTO insertCraneData(@RequestParam("uuid") String uuid, @RequestBody CraneDetailDTO crane){
        ...
        try {
            ...
            // 组装 报警状态
            if(crane.getStatus()==null){
                List<Integer> statusList = crane.getStatusList();
                if(CollectionUtils.isEmpty(statusList)){
                    return new ResultDTO(false,null,"未设置 status");
                }
    ...
    public ResultDTO insertLiftData(@RequestParam("uuid") String uuid, @RequestBody LiftDetailDTO lift){
        ...
        try {
            ...
            if(lift.getStatus()==null){
                List<Integer> statusList = lift.getStatusList();
                if(CollectionUtils.isEmpty(statusList)){
                    return new ResultDTO(false,null,"未设置 status");
                }
                if(statusList.get(0).intValue() == 0){
