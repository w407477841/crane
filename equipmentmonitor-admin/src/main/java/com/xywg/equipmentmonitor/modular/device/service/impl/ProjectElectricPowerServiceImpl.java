...
import com.xywg.equipmentmonitor.modular.device.dao.ProjectElectricPowerMapper;
...
import com.xywg.equipmentmonitor.modular.device.vo.DataVO;
import com.xywg.equipmentmonitor.modular.device.vo.ElectricAlarmVO;
import com.xywg.equipmentmonitor.modular.device.vo.ProjectElectricPowerVO;
...
//        	ew.like("b.name", res.getKey());
        ...
        if(res.getDeviceNo()!=null&&!"".equals(res.getDeviceNo()))
        {
            ew.like("a.device_no",res.getDeviceNo());
        }
        if (res.getBody() != null) {
...
//	        	ew.like("b.name", res.getKey());
    ...
    public ResultDTO<Object> updateStatus(RequestDTO<ProjectElectricPower> res) throws Exception {
        ...
        for (int i = 0; i < res.getIds().size(); i++) {
            ...
            String key = RedisConfig.DEVICE_INFO_PREFIX + "ameter:" + deviceNum;
            String redisKey = RedisConfig.DEVICE_PLATFORM + "meter:" + deviceNum;
    ...
    public ResultDTO<Object> updateInfo(RequestDTO<ProjectElectricPower> res) throws Exception {
        ...
        String key = RedisConfig.DEVICE_INFO_PREFIX + "ameter:" + baseMapper.selectById(power.getId()).getDeviceNo();
        if (redisUtil.exists(key)) {
            redisUtil.remove(key);
        }
    ...
    public boolean deleteBatchIds(List<? extends Serializable> idList) {
        for (int i = 0; i < idList.size(); i++) {
            ...
            String key = RedisConfig.DEVICE_INFO_PREFIX + "ameter:" + baseMapper.selectById(idList.get(i)).getDeviceNo();
            if (redisUtil.exists(key)) {
                redisUtil.remove(key);
            }
