...
import com.xywg.equipmentmonitor.modular.device.dao.ProjectLiftMapper;
...
import com.xywg.equipmentmonitor.modular.device.dao.ProjectEnvironmentMonitorMapper;
import com.xywg.equipmentmonitor.modular.device.dao.ProjectEnvironmentMonitorVideoMapper;
    ...
    public ResultDTO<Object> updateStatus(RequestDTO<ProjectEnvironmentMonitor> res) {
        ...
        for (int i = 0; i < res.getIds().size(); i++) {
            ...
            if("stop".equals(res.getKey())){
                ...
                String redisKey = RedisConfig.DEVICE_PLATFORM + "monitor:" + deviceNum;
...
//	        	ew.like("b.name", res.getKey());
     ...
     */
    @Override
    public ResultDTO<Object> updateDispatch(RequestDTO<ProjectEnvironmentMonitor> res) {
        ...
        for(int i=0;i<res.getIds().size();i++)
        {
            ...
            ProjectEnvironmentMonitor monitor1=baseMapper.selectById(id);
            String deviceNo=monitor1.getDeviceNo();
            redisUtil.set("xywg:yc:dispatch:"+deviceNo,1);
        }
