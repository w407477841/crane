...
public class ZbusProducerHolder {
    ...
    @Autowired
    private IProjectDeviceWorkerRecordService deviceWorkerRecordService;
    ...
    public void init() {
        ...
        list.forEach(item->{
            ...
            Producer lwtProducter = new Producer(broker);
            ConsumerConfig consumerConfig =new ConsumerConfig();
            consumerConfig.setTopic(item.getTopic()+"-0001");
            consumerConfig.setBroker(broker);
            Consumer consumer =new Consumer(consumerConfig);

            try {
                producer.declareTopic(item.getTopic());
                lwtProducter.declareTopic(item.getTopic()+"-0000");
                consumer.declareTopic(item.getTopic()+"-0001");
                consumer.start(new MessageHandler() {
                    @Override
                    public void handle(Message message, MqClient mqClient) throws IOException {
                        try {
                            BindWorkerDTO bindWorkerDTO =JSONUtil.toBean( message.getBodyString(),BindWorkerDTO.class);
                            deviceWorkerRecordService.bindWorker(bindWorkerDTO);
                        }catch (Exception e){
                            e.printStackTrace();
                        }
            ...
            producerMap.put(item.getTopic()+"-0000",lwtProducter);
        });
     ...
     * 绑定安全帽，标签后，推送至 劳务通
     */
    public void bindHelmet(String  uuid,BindHelmetVO bind)throws Exception{
        if(uuid==null){
            LOGGER.error("uuid:{} 不存在",uuid);
            return ;
        }
        ...
        Producer producer = producerMap.get(key+"-0000");
        Message msg = new Message();
        msg.setTopic(key+"-0000");
        msg.setBody(JSONUtil.toJsonStr(bind));
        if(null == producer){
