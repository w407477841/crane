...
public class Decoder extends ByteToMessageDecoder {
    ...
    protected void decode(ChannelHandlerContext ctx, ByteBuf in, List<Object> out) throws Exception {
        ...
        if(data.startsWith(HEAD_YC_DATA)){
            processor(PATTERN_YC_DATA,start,data,1,4,in,out);
        }else if(data.startsWith(HEAD_YC_HT)){
            processor(PATTERN_YC_HT,start,data,1,2,in,out);
        }else if(data.startsWith(HEAD_SJJ_DATA)){
            processor(PATTERN_SJJ_DATA,start,data,1,2,in,out);
        }else if(data.startsWith(HEAD_SJJ_HT)){
            processor(PATTERN_SJJ_HT,start,data,1,2,in,out);
        }else if(data.startsWith(HEAD_TJ_DATA)){
            processor(PATTERN_TJ_DATA,start,data,1,2,in,out);
        }else if(data.startsWith(HEAD_TJ_HT)){
            processor(PATTERN_TJ_HT,start,data,1,2,in,out);
        }else{
     ...
     * @param out 返回
     */
    private void processor(String regex,int start,String data,int fullDataGroup,int redundanceGroup,ByteBuf in,List<Object> out ){
