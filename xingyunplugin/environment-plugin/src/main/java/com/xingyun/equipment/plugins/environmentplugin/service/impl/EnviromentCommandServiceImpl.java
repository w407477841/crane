 ...
 * @author : wangyifei
 * Description
 * Date: Created in 9:00 2019/7/9
 * Modified By : wangyifei
 */
@Slf4j
public class EnviromentCommandServiceImpl extends BaseCommandService {


    public EnviromentCommandServiceImpl(CommandCallback callback) {
        super(callback);
    }

    @Override
    public void commandExec(ProtocolDTO protocolDTO,ChannelHandlerContext ctx) {
        log.info(LogEnum.COMMON.format("实时数据:",protocolDTO.toString()));
        this.callback.enviromentCallback(protocolDTO);
