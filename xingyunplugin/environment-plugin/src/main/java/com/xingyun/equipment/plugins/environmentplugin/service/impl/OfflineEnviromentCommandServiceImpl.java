 ...
 * @author : wangyifei
 * Description
 * Date: Created in 15:46 2019/7/10
 * Modified By : wangyifei
 */
@Slf4j
public class OfflineEnviromentCommandServiceImpl extends BaseCommandService {


    public OfflineEnviromentCommandServiceImpl(CommandCallback callback) {
        super(callback);
    }

    @Override
    public void commandExec(ProtocolDTO protocolDTO, ChannelHandlerContext ctx) {
        log.info(LogEnum.COMMON.format("离线实时数据:",protocolDTO.toString()));
        this.callback.offlineEnviromentCallback(protocolDTO);
